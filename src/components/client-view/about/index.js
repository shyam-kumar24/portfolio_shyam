  ...
  return (
    <div className="max-w-screen-xl mt-24 mb-6 sm:mt-14 sm:mb-14 px-6 sm:px-8 lg:px-16 mx-auto" id="about">
      <div className="w-full flex">
        <AnimationWrapper className="rounded-lg w-full grid-flow-row grid grid-cols-1 sm:grid-cols-3 py-9 divide-y-2 sm:divide-y-0 sm:divide-x-2 divide-green-500 bg-white z-10">
      ...
      <AnimationWrapper className="pt-6">
        <div className="flex flex-col justify-center items-center row-start-2 sm:row-start-1">
          ...
          <p className="text-black mt-4 mb-8 font-bold text-center">{data?.aboutme}</p>
        </div>
      </AnimationWrapper>
      ...
      <div className="grid grid-flow-row sm:grid-flow-col grid-cols-1 sm:grid-cols-2 gap-8">
        ...
        <AnimationWrapper className="flex items-center w-full p-4">
          <motion.div
            variants={setVariants}
            className="grid gap-4 grid-cols-3 h-full max-h-52 w-full"
          >
            {data?.skills.split(",").map((skill, idx) => (
              ...
              >
                <button className="whitespace-nowrap text-ellipsis overflow-hidden py-3 w-40 px-6 border-2 border-green-500 bg-white text-black font-semibold rounded-lg text-xl tracking-widest hover:shadow-md transition-all outline-none">
                  {skill}
                </button>
              </motion.div>
            ))}
