...
import AnimationWrapper from "../animation-wrapper/index";
...
export default function ClientProjectView({ data }) {
  ...
  return (
    ...
    >
      ...
      <AnimationWrapper>
        <ul
          className="flex overflow-x-scroll h-[350px] py-5 gap-5"
          ref={containerRef}
        >
          {data && data.length > 0
            ? data.map((item, index) => (
                <li
                  key={index}
                  className="flex-none w-[300px] flex items-stretch cursor-pointer"
                >
                  <div className="border-2 w-full relative border-green-500 transition-all rounded-lg flex flex-col">
                    <div className="flex p-4 flex-col xl:flex-row w-full items-stretch xl:items-center">
                      <div className="flex order-2 xl:order-1">
                        <div className="flex flex-col">
                          ...
                          <p className="text-sm mt-2 text-gray-600 capitalize font-bold">
                          ...
                          <div className="grid gap-2 mt-5 grid-cols-2 h-full max-h-[200px] w-full">
                            {item?.technologies
                              ...
                              .map((techItem, i) => (
                                ...
                                >
                                  <button className="whitespace-nowrap text-ellipsis overflow-hidden py-3 w-28 px-6 border-2 border-green-500 bg-white text-black font-semibold rounded-lg text-xs tracking-widest hover:shadow-md transition-all outline-none">
                                    {techItem}
                                  </button>
                                </div>
                              ))}
                    ...
                    <div className="absolute w-full bottom-0 justify-center flex gap-2">
                      <button
                        ...
                        className="p-2 text-white font-semibold text-[14px] tracking-widest bg-green-500 transition-all outline-none"
                      >
                        Website
                      </button>
                      <button
